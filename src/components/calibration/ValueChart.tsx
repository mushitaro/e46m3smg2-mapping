...
/**
 * The calibration data drawn rather than tabulated, in the two flat forms: HEAT is the field seen
 * from above, 2D is one section through it. Ported verbatim from the reference tuner's
 * `ValueChart.tsx` so a map looks the same wherever it is opened.
 *
 * Fed from the LOADED BINARY's decode and drawn as plain SVG, so it costs nothing to mount.
 */
 ...
 * The TABLE's heat underlay uses the same function, so the two views agree about which end is hot.
 */
...
/**
 * A signed quantity — a difference — on the house's diverging convention: blue below, red above,
 * both fading to the panel at zero. Anchored on 0 rather than on the data's own range, because
 * "no change" has to look like nothing whichever way the rest of the field leans.
 */
 ...
 *
 * The unit is part of that number, not a caption on it — 0.85 is not a reading until it says
 * mbar — so it is sized and coloured to be read beside the value. `raw` stays dim because it
 * genuinely is the aside: the count in the binary, for checking the scaling against.
 */
export function ScalarReadout({ value, raw, units }: { value: number | null; raw: number; units?: string | null }) {
    return (
        <div className="flex items-baseline gap-2 p-3">
            <span className="font-mono text-2xl text-blue-400">{fmtValue(value)}</span>
            {units && units !== '-' && (
                <span className="font-mono text-sm text-slate-300">{units}</span>
            )}
            <span className="font-mono text-[9px] text-slate-500">raw {raw}</span>
        </div>
    );
}
 ...
 *
 * Clicking a cell moves the shared selection, so the section below, the table and the cell editor
 * are all looking at the same place. One window, one index.
 */
    ...
    return (
        ...
        >
            ...
            {selected && (
                ...
                <rect
                    className="pointer-events-none fill-none stroke-slate-100"
                    strokeWidth={1.2}
...
/**
 * One section, as a line — and the reference beside it when the compare bar names a different
 * one, which is what makes "what did this change" readable without arithmetic.
 */
