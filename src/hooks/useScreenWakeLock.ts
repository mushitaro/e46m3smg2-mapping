 ...
 * USB connection dropped, in the middle of a read. This tool has no write path, so the window it
 * protects is a full-image read: 4,370 telegrams, roughly eighteen minutes, on a phone lying on
 * the passenger seat. That is well past any inactivity timeout.
 *
 * Ported byte-for-byte from the reference tuner apart from this paragraph. The discipline in it —
 * closure variable rather than state, re-acquire on `visibilitychange`, best-effort throughout —
 * was worked out there and is not worth re-deriving.
 *
 * ## What it is not
 *
