 ...
 *
 * Ported verbatim from the reference tuner (`E46M3CSL_TuningTool/src/hooks/useFitScale.ts`) — both
 * the mechanism and the two measurements written into it. This app had no auto-fit at all, which is
 * why its control panel was a stack of hand-tuned `min-h` clamps that still could not promise the
 * ring would be whole on a short landscape phone. Measuring the box and scaling to it is the
 * system's own answer (`ux-patterns.md` -> Auto-fit scaling); the clamps were a substitute for it.
 *
